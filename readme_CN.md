...
可复用的异步信号——类似 `Promise.withResolvers()`，但可重置、可观察、原生支持中止
...
`asyncSignal`支持集成`AbortSignal`。

- 创建`asyncSignal`时允许传入一个`AbortSignal`,当`AbortSignal`中止时，会自动中止`asyncSignal`。

- 使用 `abortAt` 选项可以当`AbortSignal`中止时`asyncSignal`的行为。
  ...
  abortSignal1.addEventListener("abort", () => {
      // 当asyncSignal resolve、reject 和 reset 时均导致abortSignal1中止
  });
  
  // 仅在 reject 时中止（适用于网络请求）
  const signal2 = asyncSignal({ abortAt: "reject" });
  const abortSignal2 = signal1.getAbortSignal()
  abortSignal2.addEventListener("abort", () => {
      // 仅在asyncSignal reject时才导致abortSignal2中止
  });
  
  
  // 仅在 resolve 时中止（适用于资源清理）
  const signal3 = asyncSignal({ abortAt: "resolve" });
  const abortSignal3 = signal1.getAbortSignal()
  abortSignal3.addEventListener("abort", () => {
      // 仅在asyncSignal resolve时才导致abortSignal3中止
  });
  ...
  **方法说明：**
  ...
  ```typescript
  const loader = new AsyncLoader((args) => fetch("/api/data", { signal: args.abortSignal }), {
